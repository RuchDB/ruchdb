...
use crate::{BYTE_ALIGN_SIZE, size_of, align_of, size_of_aligned};
...
mod layout_tests {
    ...
    fn calc_layout_of_required_bytes() {
        ...
        if cfg!(target_pointer_width = "32") {
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(1))), Layout::from_size_align(4, 1));
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(3))), Layout::from_size_align(4, 1));
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(4))), Layout::from_size_align(4, 1));
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(5))), Layout::from_size_align(8, 1));
        }
        if cfg!(target_pointer_width = "64") {
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(1))), Layout::from_size_align(8, 1));
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(3))), Layout::from_size_align(8, 1));
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(4))), Layout::from_size_align(8, 1));
            assert_eq!(Ok(layout_of_bytes(size_of_sys_aligned(5))), Layout::from_size_align(8, 1));
        }
    }
